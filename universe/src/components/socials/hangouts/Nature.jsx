...
const Nature = () => {
  ...
  return (
    <div className="bg-gray-900 text-white min-h-screen p-6">
      <h1 className="text-3xl font-bold text-center mb-8">Nature</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {natures.map((nature) => (
          <div
            key={nature._id}
            onClick={() => fetchReviews(nature.name)}
          ...
          >
            <img
              src={`data:image/jpeg;base64,${nature.image}`}
              alt={nature.name}
              className="w-full h-48 object-cover"
            />
            <div className="p-4">
              <h2 className="text-xl font-bold">{nature.name}</h2>
              <p className="text-sm text-gray-400 flex items-center">
                ⭐
                Rating: {nature.avg_rating}
              </p>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white text-black rounded-lg p-6 w-3/4 max-w-lg relative">
            ...
            <div className="mb-6">
              ...
              <img
                src={`data:image/jpeg;base64,${natures.find(r => r.name === selectedPlace)?.image}`}
                alt={selectedPlace}
